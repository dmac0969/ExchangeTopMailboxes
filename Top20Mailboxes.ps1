...
{ 
    ...
    $usafaMailboxes = Get-Mailbox -resultsize unlimited | where {$_.CustomAttribute1 -ne "Org"} | Get-MailboxStatistics | sort TotalItemSize -Descending | select -First $numberOfMailboxes 
}
else 
{ 
...
{ 
    ...
    # Create custom object based on the object hash that we created above. Each separate value will be populated below.
